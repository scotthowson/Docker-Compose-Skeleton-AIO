...
handle_config_update() {
    ...
    local -A allowed_keys=(
        ...
        # Notifications
        [NTFY_URL]=1 [NTFY_TOPIC]=1 [NTFY_PRIORITY]=1 [NTFY_TOKEN]=1 [CROWDSEC_TRUSTED_IPS]=1
...
_fire_notifications() {
    ...
    while IFS= read -r rule; do
        ...
        # Send via NTFY (background, non-blocking) to NTFY_URL/NTFY_TOPIC
        (
            local _result
            _result=$(_ntfy_send "$title" "$message" "$priority" "$tags_str")
...
handle_notification_test() {
    ...
    local result
    result=$(_ntfy_send "$title" "$message" "$priority" "$tags")
    ...
    if [[ "$result" =~ ^2 ]]; then
        _api_success "{\"success\": true, \"message\": \"Test notification sent to $(_api_json_escape "$(_ntfy_endpoint)")\", \"status_code\": $result, \"timestamp\": \"$ts\"}"
    else
        _api_error 502 "ntfy at $(_ntfy_endpoint) answered $result (0 = unreachable). Check NTFY_URL, NTFY_TOPIC and, if the server requires it, NTFY_TOKEN."
    fi
}
...
" > "$target_dir/docker-compose.yml" 2>/dev/null || true
    ...
    if [[ "$auto_start" == "true" ]]; then
        ...
        (
            ...
            _compose_with_secrets "$target_dir/docker-compose.yml" "$_deploy_env" up -d --no-recreate $_svc_list >/dev/null 2>&1 || _up_ok=false
    ...
    _api_success "{\"success\": true, \"target_stack\": \"$(_api_json_escape "$target_stack")\", \"services_added\": $services_json, \"started\": $started, \"warning\": \"$(_api_json_escape "$deploy_warning")\", \"backup_file\": \"docker-compose.yml.bak.${timestamp}\", \"message\": \"Template services merged into $target_stack successfully\"}"
}
...
_automation_execute() {
    ...
    case "$action" in
        ...
        notification_send)
            ...
            local code
            code=$(_ntfy_send "DCS: ${name}" "$text" default "robot")
    ...
    (
        ...
        case "$action" in
            start)
                ...
                _compose_with_secrets "$compose_file" "$env_file" up -d --remove-orphans && ok=true || ok=false
            ...
            stop)
                ...
                $DOCKER_COMPOSE_CMD "${args[@]}" down --remove-orphans --timeout 15 && ok=true || ok=false
