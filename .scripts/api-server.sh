...
# GET /routes/certificates — TLS state of the reverse proxy: ACME challenge, account email, the certificates Traefik holds and its recent ACME errors, with hints
handle_routes_certificates() {
    ...
    _api_success "{\"traefik_stack\": \"$(_api_json_escape "$stack")\", \"active\": true, \"challenge\": \"$challenge\", \"email\": \"$(_api_json_escape "$email")\", \"token_set\": $token_set, \"acme_file\": {\"exists\": $exists, \"mode\": \"$mode\", \"mode_ok\": $mode_ok}, \"certificates\": $certs, \"errors\": $acme_errors, \"hints\": $hints_json, \"checked_at\": \"$(date -u '+%Y-%m-%dT%H:%M:%SZ')\"}"
}
