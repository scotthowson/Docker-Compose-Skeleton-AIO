...
API_CAPS_FILE="${BASE_DIR}/.data/api-server.caps"
...
handle_health() {
    ...
    _api_success "{\"status\": \"$overall\", \"summary\": {\"total\": $total, \"healthy\": $healthy, \"unhealthy\": $unhealthy, \"stopped\": $stopped, \"sleeping\": $sleeping}, \"containers\": $containers_json, \"api\": {\"uptime_seconds\": $api_uptime, \"requests_total\": ${api_requests:-0}, \"errors_total\": ${api_errors:-0}, \"memory_kb\": ${api_mem_kb:-0}, \"pid\": ${api_pid_val:-0}}}"
}
...
handle_config() {
    ...
    config+="\"update_channel\": \"$(_api_update_channel)\","
...
handle_config_update() {
    ...
    local -A allowed_keys=(
        ...
        # Notifications
        [NTFY_URL]=1 [NTFY_TOPIC]=1 [NTFY_PRIORITY]=1 [NTFY_TOKEN]=1 [DISCORD_WEBHOOK_URL]=1 [UPDATE_ON_BOOT]=1 [UPDATE_CHANNEL]=1 [CROWDSEC_TRUSTED_IPS]=1
    ...
    # A DDNS change applies at once (the loop otherwise only starts with the listener)
...
# =============================================================================
# SELF-UPDATE — release channels, user files kept, restart without root
# =============================================================================

# Paths git tracks but users legitimately edit (deployed stacks, template
# files, alert/automation state, plugin manifests). An update never replaces
# them: a locally changed file is put back byte for byte after the switch,
# a locally deleted one stays deleted.
_API_GIT_USER_PATHS=(Stacks .templates .api-auth .plugins)

_api_git_is_user_path() {
    local p="$1" u
    for u in "${_API_GIT_USER_PATHS[@]}"; do
        [[ "$p" == "$u" || "$p" == "$u/"* ]] && return 0
    done
    return 1
}

# Release channel from .env: stable (newest vX.Y.Z tag, the default) or main.
_api_update_channel() {
...
# Newest release tag on origin, read from the remote without fetching objects.
_api_update_latest_tag() {
    timeout 25 git ls-remote --tags --refs origin 'v[0-9]*' 2>/dev/null \
        | awk '{print $2}' | sed 's|^refs/tags/||' \
        | grep -E '^v[0-9]+\.[0-9]+\.[0-9]+$' | sort -V | tail -1 || true
}

# Resolve what the channel points at. Sets UPD_CHANNEL, UPD_TARGET (full
# commit hash), UPD_TARGET_NAME ("v3.3.0" or "main"), UPD_TARGET_COMMIT
# (short), UPD_TARGET_VERSION (its VERSION file) and UPD_ERROR ("" when the
# remote answered). Returns 1 when the remote could not be read.
_api_update_resolve_target() {
    UPD_CHANNEL=$(_api_update_channel)
    UPD_TARGET="" UPD_TARGET_NAME="" UPD_TARGET_COMMIT="" UPD_TARGET_VERSION="" UPD_ERROR=""
    local out="" tag=""
    if [[ "$UPD_CHANNEL" == "stable" ]]; then
        tag=$(_api_update_latest_tag)
        if [[ -z "$tag" ]]; then
            UPD_ERROR="Could not list releases on GitHub (offline, or github.com is unreachable from this host)"
            return 1
        fi
        UPD_TARGET_NAME="$tag"
        ...
        if ! out=$(timeout 90 git fetch --no-tags origin '+refs/heads/main:refs/remotes/origin/main' 2>&1); then
            UPD_ERROR="Could not fetch main from GitHub: $(printf '%s' "$out" | tail -1)"
            return 1
        fi
        UPD_TARGET_NAME="main"
        UPD_TARGET=$(git rev-parse refs/remotes/origin/main 2>/dev/null || true)
    fi
    if [[ -z "$UPD_TARGET" ]]; then
        UPD_ERROR="Could not resolve $UPD_TARGET_NAME after fetching it"
        return 1
    fi
    UPD_TARGET_COMMIT=$(git rev-parse --short "$UPD_TARGET" 2>/dev/null || echo "${UPD_TARGET:0:7}")
    UPD_TARGET_VERSION=$(git show "$UPD_TARGET:VERSION" 2>/dev/null | tr -d '[:space:]' || true)
    return 0
}

# Relation between HEAD and UPD_TARGET: current | behind | ahead | diverged
_api_update_state() {
    local head
    head=$(git rev-parse HEAD 2>/dev/null || true)
    if [[ "$head" == "$UPD_TARGET" ]]; then echo current
    elif git merge-base --is-ancestor HEAD "$UPD_TARGET" 2>/dev/null; then echo behind
    elif git merge-base --is-ancestor "$UPD_TARGET" HEAD 2>/dev/null; then echo ahead
    else echo diverged
    fi
}

# CHANGELOG sections newer than the installed version, read from the target.
_api_update_release_notes() {
    [[ -n "${UPD_TARGET:-}" ]] || return 0
    git show "$UPD_TARGET:CHANGELOG.md" 2>/dev/null | awk -v cur="[${DCS_VERSION:-}]" '
        /^## \[/ { if (index($0, cur)) exit; started=1 }
        started { print }
    ' | head -c 12000 || true
}
...
_api_update_scan_local() {
    ...
    done < <(git diff --name-only HEAD "$UPD_TARGET" 2>/dev/null || true)
}

# Move the checkout to UPD_TARGET (MODE ff = fast-forward merge, reset =
# git reset --hard) while user-owned files stay exactly as they are.
# Framework files with local edits are replaced only when REPLACE is true;
# the previous copies are kept under .data/update-backups/<stamp>/.
# Sets UPD_BACKUP_DIR, UPD_KEPT, UPD_REPLACED, UPD_SWITCH_OUTPUT.
# Returns 0 on success, 2 when framework edits block the switch, 1 on a
# git failure (nothing changed, user files back in place).
_api_update_switch() {
    local mode="$1" replace="${2:-false}"
    UPD_KEPT=() UPD_REPLACED=() UPD_BACKUP_DIR="" UPD_SWITCH_OUTPUT=""
    _api_update_scan_local
    if [[ ${#UPD_FW_CONFLICT[@]} -gt 0 && "$replace" != "true" ]]; then
        UPD_SWITCH_OUTPUT="local changes to framework files would be overwritten: ${UPD_FW_CONFLICT[*]}"
        return 2
    fi
    # A lock left behind by an interrupted git run blocks every later one
    ...
    for f in ${UPD_USER_KEEP[@]+"${UPD_USER_KEEP[@]}"} ${UPD_FW_CONFLICT[@]+"${UPD_FW_CONFLICT[@]}"}; do
        [[ -n "$f" ]] || continue
        mkdir -p "$UPD_BACKUP_DIR/$(dirname "$f")" 2>/dev/null || true
        if [[ -e "$f" ]]; then
            cp -a -- "$f" "$UPD_BACKUP_DIR/$f" 2>/dev/null || true
        else
            printf '%s\n' "$f" >> "$UPD_BACKUP_DIR/.deleted"
        fi
    done
    # 2. a clean tree for those paths, so git has nothing to refuse
    for f in ${UPD_USER_KEEP[@]+"${UPD_USER_KEEP[@]}"} ${UPD_FW_CONFLICT[@]+"${UPD_FW_CONFLICT[@]}"}; do
        [[ -n "$f" ]] || continue
        st="${UPD_DIRTY_STATUS[$f]:-}"
        if [[ "$st" == "??" ]]; then
    ...
    local rc=0
    if [[ "$mode" == "reset" ]]; then
        UPD_SWITCH_OUTPUT=$(git reset -q --hard "$UPD_TARGET" 2>&1) || rc=$?
    else
    ...
    for f in ${UPD_USER_KEEP[@]+"${UPD_USER_KEEP[@]}"}; do
        [[ -n "$f" ]] || continue
        if [[ -e "$UPD_BACKUP_DIR/$f" ]]; then
            mkdir -p "$(dirname "$f")" 2>/dev/null || true
            cp -a -- "$UPD_BACKUP_DIR/$f" "$f" 2>/dev/null || true
        else
            rm -rf -- "$f" 2>/dev/null || true
        fi
        UPD_KEPT+=("$f")
    ...
    if [[ $rc -ne 0 ]]; then
        for f in ${UPD_FW_CONFLICT[@]+"${UPD_FW_CONFLICT[@]}"}; do
            [[ -n "$f" && -e "$UPD_BACKUP_DIR/$f" ]] || continue
            cp -a -- "$UPD_BACKUP_DIR/$f" "$f" 2>/dev/null || true
        done
        return 1
    fi
    ...
    # the last ten backup sets are enough
    ls -1d "$BASE_DIR"/.data/update-backups/*/ 2>/dev/null | sort | head -n -10 | xargs -r rm -rf 2>/dev/null || true
    return 0
}
...
_upd_json_list() {
    ...
    for x in "$@"; do
        ...
        $first || out+=","
        first=false
        out+="\"$(_api_json_escape "$x")\""
    done
    printf '%s]' "$out"
}

_upd_json_commits() {
    # commits RANGE as a JSON array (tab-separated log, every field escaped)
    local range="$1" out="[" first=true _h _m _a _d
    while IFS=$'\t' read -r _h _m _a _d; do
        [[ -z "$_h" ]] && continue
        $first || out+=","
        first=false
        out+="{\"hash\":\"$(_api_json_escape "$_h")\",\"message\":\"$(_api_json_escape "$_m")\",\"author\":\"$(_api_json_escape "$_a")\",\"date\":\"$(_api_json_escape "$_d")\"}"
    done < <(git log "$range" --pretty=format:'%h%x09%s%x09%an%x09%ci' 2>/dev/null | head -40 || true)
    printf '%s]' "$out"
}
...
_upd_restart_json() {
    printf '{"method": "%s", "eta_seconds": %s, "hint": "%s"}' \
        "$UPD_RESTART_METHOD" "${UPD_RESTART_ETA:-0}" "$(_api_json_escape "$UPD_RESTART_HINT")"
}

# GET /system/update/check — Newer DCS release on the channel? Version, release notes, local edits and how the API can restart
handle_system_update_check() {
    ...
    local current_commit branch checked=true error="" state="unknown" behind=0 changelog="[]" notes=""
    current_commit=$(git rev-parse --short HEAD 2>/dev/null || echo "unknown")
    branch=$(git rev-parse --abbrev-ref HEAD 2>/dev/null || echo "unknown")

    if _api_update_resolve_target; then
        state=$(_api_update_state)
        behind=$(git rev-list --count HEAD.."$UPD_TARGET" 2>/dev/null || echo 0)
        [[ "$behind" =~ ^[0-9]+$ ]] || behind=0
        changelog=$(_upd_json_commits "HEAD..$UPD_TARGET")
    ...
    local available=false has_conflicts=false
    [[ "$state" == "behind" ]] && available=true
  ...
  \"state\": \"$state\",
  \"checked\": $checked,
  \"error\": \"$(_api_json_escape "$error")\",
  \"channel\": \"$(_api_json_escape "${UPD_CHANNEL:-stable}")\",
  \"branch\": \"$(_api_json_escape "$branch")\",
  \"current_version\": \"$(_api_json_escape "${DCS_VERSION:-unknown}")\",
  \"current_commit\": \"$(_api_json_escape "$current_commit")\",
  \"latest_version\": \"$(_api_json_escape "${UPD_TARGET_VERSION:-}")\",
  \"latest_name\": \"$(_api_json_escape "${UPD_TARGET_NAME:-}")\",
  \"latest_commit\": \"$(_api_json_escape "${UPD_TARGET_COMMIT:-}")\",
  \"commits_behind\": $behind,
  \"changelog\": $changelog,
  \"release_notes\": \"$(_api_json_escape "$notes")\",
  \"local_changes\": {
    \"user\": $(_upd_json_list ${UPD_USER_DIRTY[@]+"${UPD_USER_DIRTY[@]}"}),
    \"framework\": $(_upd_json_list ${UPD_FW_DIRTY[@]+"${UPD_FW_DIRTY[@]}"}),
    \"kept\": $(_upd_json_list ${UPD_USER_KEEP[@]+"${UPD_USER_KEEP[@]}"}),
    \"conflicts\": $(_upd_json_list ${UPD_FW_CONFLICT[@]+"${UPD_FW_CONFLICT[@]}"})
  },
  \"has_local_changes\": $has_conflicts,
...
# POST /system/update/apply — Update to the channel's release {confirm, replace_local, restart}; user files are kept, a backup tag allows rollback
handle_system_update_apply() {
    ...
    local confirm replace_local restart_after
    confirm=$(printf '%s' "$body" | jq -r '.confirm // empty' 2>/dev/null || true)
    if [[ "$confirm" != "true" ]]; then
        _api_error 400 "Missing confirmation. Send {\"confirm\": true} to apply the update."
        return
    fi
    replace_local=$(printf '%s' "$body" | jq -r 'if (.replace_local == true or .replace_local == "true") then "true" else "false" end' 2>/dev/null || echo false)
    restart_after=$(printf '%s' "$body" | jq -r 'if (.restart == true or .restart == "true") then "true" else "false" end' 2>/dev/null || echo false)

    if ! _api_update_resolve_target; then
        _api_error 502 "$UPD_ERROR"
        return
    fi

    local branch current state
    branch=$(git rev-parse --abbrev-ref HEAD 2>/dev/null || echo "unknown")
    ...
    if [[ "$state" != "behind" ]]; then
        local why
        case "$state" in
            current)  why="Already up to date ($UPD_TARGET_NAME)" ;;
            ahead)    why="This install is ahead of $UPD_TARGET_NAME (local commits); nothing to apply" ;;
            *)        why="This install's history diverged from $UPD_TARGET_NAME; align it by hand (git fetch origin && git reset --hard $UPD_TARGET_COMMIT) after saving local work" ;;
        esac
        _api_success "{
  \"updated\": false,
  \"state\": \"$state\",
  \"message\": \"$(_api_json_escape "$why")\",
  \"current_version\": \"$(_api_json_escape "${DCS_VERSION:-unknown}")\",
  \"current_commit\": \"$(_api_json_escape "$current")\",
  \"branch\": \"$(_api_json_escape "$branch")\"
}"
        return
    fi
    ...
    _api_update_scan_local
    if [[ ${#UPD_FW_CONFLICT[@]} -gt 0 && "$replace_local" != "true" ]]; then
        _api_error 409 "Local changes to framework files would be overwritten: ${UPD_FW_CONFLICT[*]}. Send replace_local=true to replace them (copies are kept under .data/update-backups)."
        return
    fi
    ...
    changelog=$(_upd_json_commits "${backup_tag}..HEAD")
    if git diff --name-only "$backup_tag" HEAD -- .scripts/install-service.sh 2>/dev/null | grep -q .; then
        service_changed=true
    fi
    # settings that exist in the new .env.example but not in .env (defaults apply)
    new_settings=$(comm -23 \
        <(grep -oE '^[A-Z_][A-Z0-9_]*=' "$BASE_DIR/.env.example" 2>/dev/null | tr -d '=' | sort -u) \
        <(grep -oE '^[A-Z_][A-Z0-9_]*=' "$BASE_DIR/.env" 2>/dev/null | tr -d '=' | sort -u) 2>/dev/null | tr '\n' ' ' || true)

    _audit_log "SYSTEM_UPDATE" "${DCS_VERSION:-?}→$new_version ($current→$new_commit, channel $UPD_CHANNEL, kept ${#UPD_KEPT[@]} user files, replaced ${#UPD_REPLACED[@]})"
  ...
  \"updated\": true,
  \"state\": \"updated\",
  \"channel\": \"$(_api_json_escape "$UPD_CHANNEL")\",
  \"branch\": \"$(_api_json_escape "$branch")\",
  \"previous_version\": \"$(_api_json_escape "${DCS_VERSION:-unknown}")\",
  \"previous_commit\": \"$(_api_json_escape "$current")\",
  \"new_version\": \"$(_api_json_escape "$new_version")\",
  \"new_commit\": \"$(_api_json_escape "$new_commit")\",
  \"backup_tag\": \"$(_api_json_escape "$backup_tag")\",
  \"commits_applied\": $behind,
  \"changelog\": $changelog,
  \"release_notes\": \"$(_api_json_escape "$notes")\",
  \"kept_local\": $(_upd_json_list ${UPD_KEPT[@]+"${UPD_KEPT[@]}"}),
  \"replaced_local\": $(_upd_json_list ${UPD_REPLACED[@]+"${UPD_REPLACED[@]}"}),
  \"backup_dir\": \"$(_api_json_escape "${UPD_BACKUP_DIR:-}")\",
  \"new_settings\": $(_upd_json_list ${new_settings}),
  \"service_definition_changed\": $service_changed,
  \"restart_required\": true,
  \"restart_scheduled\": $([[ "$restart_after" == "true" && "$UPD_RESTART_METHOD" != "manual" ]] && echo true || echo false),
  \"restart\": $(_upd_restart_json),
  \"message\": \"Updated to $(_api_json_escape "$new_version")$( [[ "$restart_after" == "true" && "$UPD_RESTART_METHOD" != "manual" ]] && printf '; the API restarts now' || printf '; restart the API to load it everywhere' )\"
}"
}

# POST /system/update/rollback — Return to a backup tag {backup_tag, restart}; user files are kept, edited framework files backed up
handle_system_update_rollback() {
    ...
    local backup_tag="" restart_after=false
    if command -v jq >/dev/null 2>&1 && [[ -n "$body" ]]; then
        backup_tag=$(printf '%s' "$body" | jq -r '.backup_tag // empty' 2>/dev/null || true)
        restart_after=$(printf '%s' "$body" | jq -r 'if (.restart == true or .restart == "true") then "true" else "false" end' 2>/dev/null || echo false)
    fi
    ...
    # Only our own tags: never an arbitrary checkout
    ...
    if ! git rev-parse -q --verify "$backup_tag^{commit}" >/dev/null 2>&1; then
    ...
    local rolled_back_to restored_version
    rolled_back_to=$(git rev-parse --short HEAD 2>/dev/null || echo "unknown")
    restored_version=$(tr -d '[:space:]' < "$BASE_DIR/VERSION" 2>/dev/null || echo "unknown")
    _audit_log "SYSTEM_ROLLBACK" "$current→$rolled_back_to ($backup_tag)"
  ...
  \"restored_version\": \"$(_api_json_escape "$restored_version")\",
  \"restored_commit\": \"$(_api_json_escape "$rolled_back_to")\",
  \"backup_tag\": \"$(_api_json_escape "$backup_tag")\",
  \"branch\": \"$(_api_json_escape "$branch")\",
  \"kept_local\": $(_upd_json_list ${UPD_KEPT[@]+"${UPD_KEPT[@]}"}),
  \"replaced_local\": $(_upd_json_list ${UPD_REPLACED[@]+"${UPD_REPLACED[@]}"}),
  ...
  \"restart_scheduled\": $([[ "$restart_after" == "true" && "$UPD_RESTART_METHOD" != "manual" ]] && echo true || echo false),
  \"restart\": $(_upd_restart_json),
  \"message\": \"Rolled back to $(_api_json_escape "$restored_version"). Restart the API to load it everywhere.\"
}"
}
...
handle_system_restart() {
    ...
    if [[ "$UPD_RESTART_METHOD" == "manual" ]]; then
        ...
        _api_success "{\"restarting\": true, \"method\": \"$UPD_RESTART_METHOD\", \"eta_seconds\": $UPD_RESTART_ETA, \"hint\": \"$(_api_json_escape "$UPD_RESTART_HINT")\"}"
    fi
}
...
# Start the DDNS loop when its settings allow it and it is not running, stop it
# when it was switched off. Called after the wizard and after a config change,
# so DDNS never waits for the next API restart. The loop is detached from the
# request that started it.
_ddns_ensure_running() {
    _api_load_env_file "$BASE_DIR/.env"
    local pid="" running=false
    [[ -f "$DDNS_PID_FILE" ]] && pid=$(cat "$DDNS_PID_FILE" 2>/dev/null)
    if [[ -n "$pid" ]] && kill -0 "$pid" 2>/dev/null; then running=true; fi
    local domain="${TRAEFIK_DOMAIN:-${PROXY_DOMAIN:-}}"
    if [[ "${DDNS_ENABLED:-false}" == "true" && -n "${CF_DNS_API_TOKEN:-}" && -n "$domain" && "$domain" != "example.com" ]]; then
        [[ "$running" == "true" ]] && return 0
        ( TRAEFIK_DOMAIN="$domain" _ddns_update_loop ) </dev/null >/dev/null 2>&1 &
        echo $! > "$DDNS_PID_FILE" 2>/dev/null || true
        _audit_log "DDNS_STARTED" "domain=$domain subdomains=${DDNS_SUBDOMAINS:-@}" 2>/dev/null || true
    elif [[ "$running" == "true" ]]; then
        kill "$pid" 2>/dev/null || true
        rm -f "$DDNS_PID_FILE" 2>/dev/null || true
        _audit_log "DDNS_STOPPED" "settings changed" 2>/dev/null || true
    fi
    return 0
}
...
# _traefik_pick_challenge FILE dns|http — keep the ACME challenge block the
# deploy asked for in a traefik.yml carrying the dcs-challenge markers: the
# chosen block is uncommented, the other one commented out. Idempotent.
_traefik_pick_challenge() {
    local file="$1" mode="${2:-dns}" out
    [[ -f "$file" ]] || return 0
    grep -q '# dcs-challenge: dns' "$file" 2>/dev/null || return 0
    out=$(awk -v mode="$mode" '
        /# dcs-challenge: dns/  { blk="dns";  print; next }
        /# dcs-challenge: http/ { blk="http"; print; next }
        /# dcs-challenge: end/  { blk="";     print; next }
        blk != "" {
            match($0, /^[ \t]*/); ind=substr($0, 1, RLENGTH); rest=substr($0, RLENGTH+1)
            if (blk == mode) { sub(/^# ?/, "", rest) } else if (rest !~ /^#/) { rest="# " rest }
            print ind rest; next
        }
        { print }' "$file") || return 1
...
# GET /routes/certificates — Reverse-proxy health: domain, ACME challenge and account, certificates held, a live probe of every route through Traefik, the last Traefik errors, and hints
handle_routes_certificates() {
    local stack="" ad="" line
    if line=$(_traefik_stack_appdata); then stack="${line%%	*}"; ad="${line#*	}"; fi
    if [[ -z "$stack" ]]; then
        _api_success '{"traefik_stack": "", "active": false, "challenge": "none", "email": "", "token_set": false, "acme_file": {"exists": false, "mode": "", "mode_ok": false}, "certificates": [], "errors": [], "hints": ["No stack runs Traefik yet: deploy the traefik template to get certificates."]}'
        return
    fi
    local cfg="$ad/Traefik/traefik.yml" acme="$ad/Traefik/acme.json"
    local challenge="unknown" email="" token_set=false exists=false mode="" mode_ok=false
    if [[ -f "$cfg" ]]; then
        if grep -qE '^[[:space:]]+dnsChallenge:' "$cfg"; then challenge="dns"; elif grep -qE '^[[:space:]]+httpChallenge:' "$cfg"; then challenge="http"; fi
        email=$(sed -nE 's/^[[:space:]]+email:[[:space:]]*//p' "$cfg" | head -1 | tr -d '"'"'"'"')
    fi
    grep -qE '^CF_DNS_API_TOKEN=.+' "$COMPOSE_DIR/$stack/.env" 2>/dev/null && token_set=true
    if [[ -f "$acme" ]]; then
        exists=true; mode=$(stat -c %a "$acme" 2>/dev/null); [[ "$mode" == "600" ]] && mode_ok=true
    fi
    # Certificates: acme.json keeps each chain as base64 PEM; openssl reads the leaf
    local certs="[]" now
    now=$(date +%s)
    if [[ -s "$acme" ]] && command -v openssl >/dev/null 2>&1; then
        local resolver main sans b64 not_after na_epoch days
        certs=$(while IFS=$'\t' read -r resolver main sans b64; do
            [[ -n "$b64" ]] || continue
            not_after=$(printf '%s' "$b64" | base64 -d 2>/dev/null | openssl x509 -noout -enddate 2>/dev/null | sed 's/^notAfter=//')
            na_epoch=$(date -d "$not_after" +%s 2>/dev/null || echo 0)
            days=$(( (na_epoch - now) / 86400 ))
            jq -nc --arg r "$resolver" --arg m "$main" --arg s "$sans" --arg na "$not_after" --argjson d "$days" \
                '{resolver: $r, domain: $m, sans: ($s | split(",") | map(select(length > 0))), not_after: $na, days_left: $d}'
        done < <(jq -r 'to_entries[] | .key as $r | (.value.Certificates // [])[] | [$r, (.domain.main // ""), ((.domain.sans // []) | join(",")), (.certificate // "")] | @tsv' "$acme" 2>/dev/null) | jq -sc '.' 2>/dev/null) || certs="[]"
        [[ -n "$certs" ]] || certs="[]"
    fi
    # Recent ACME trouble straight from Traefik's log
    local acme_errors="[]"
    if docker inspect Traefik >/dev/null 2>&1; then
        acme_errors=$(timeout 15 docker logs Traefik --since 48h 2>&1 | grep -i -E 'acme|certificate|letsencrypt' | grep -i -E 'error|unable|fail|invalid|denied|timeout|too open' | tail -6 | cut -c1-400 | jq -R . 2>/dev/null | jq -sc '.' 2>/dev/null) || acme_errors="[]"
        [[ -n "$acme_errors" ]] || acme_errors="[]"
    fi
    # The domain routes are built on, the live probe and Traefik's own complaints:
    # a route whose middleware or service does not exist is dropped and answers 404,
    # and only the log says why
    local domain; domain=$(_find_traefik_domain)
    local probe="null"
    if [[ -x "$BASE_DIR/.scripts/proxy-reconcile.sh" ]]; then
        probe=$(timeout 60 "$BASE_DIR/.scripts/proxy-reconcile.sh" --json --dry-run 2>/dev/null | jq -c '{routes, passing, skipped_target_down, dead, backend_down}' 2>/dev/null) || probe="null"
        [[ -n "$probe" ]] || probe="null"
    fi
    local proxy_log="[]"
    if docker inspect Traefik >/dev/null 2>&1; then
        proxy_log=$(timeout 15 docker logs Traefik --since 24h 2>&1 | grep -E 'level=(error|warn)|ERR|WRN' | grep -v -i 'aliasHeadersStrategy' | tail -8 | cut -c1-300 | jq -R . 2>/dev/null | jq -sc '.' 2>/dev/null) || proxy_log="[]"
        [[ -n "$proxy_log" ]] || proxy_log="[]"
    fi
    ...
    if [[ "$n_dead" =~ ^[0-9]+$ && "$n_dead" -gt 0 ]]; then
        if printf '%s' "$probe" | jq -e '(.dead // []) | any(endswith("=404"))' >/dev/null 2>&1; then
            hints+=("Traefik answers 404 for $n_dead route(s): it has no router for that name. Usually a middleware or service the route file references does not exist (the Traefik log below says which), the route file names another domain, or Traefik is not reading its routes directory.")
        else
            hints+=("$n_dead route(s) do not answer through Traefik; see the probe and the Traefik log below.")
        fi
    fi
    [[ "$challenge" == "dns" && "$token_set" == "false" ]] && hints+=("The DNS challenge is selected but CF_DNS_API_TOKEN is empty in the $stack stack: store a Cloudflare token (Zone:DNS:Edit) there and restart Traefik, or redeploy the traefik template without a token to use the HTTP challenge.")
    [[ "$exists" == "true" && "$mode_ok" == "false" ]] && hints+=("acme.json must be mode 600 (it is $mode); Traefik refuses to use it otherwise.")
    [[ "$email" == *example.com* || -z "$email" ]] && hints+=("The ACME account email is '$email'; Let's Encrypt rejects example.com addresses. Set TRAEFIK_ACME_EMAIL and redeploy, or edit traefik.yml.")
...
# `sablier:` plugin block in the route files (scalar `names: X` or a list).
_sablier_names() {
    ...
    { find "$dir" -type f \( -name '*.yml' -o -name '*.yaml' \) -print0 2>/dev/null
      find "$(dirname "$dir")" -maxdepth 1 -type f \( -name '*.yml' -o -name '*.yaml' \) ! -name 'traefik.yml' -print0 2>/dev/null; } | xargs -0 awk '
        ...
        insab && /^[ \t]*names:[ \t]*/ {
            v=$0; sub(/^[ \t]*names:[ \t]*/, "", v); gsub(/["\x27]/, "", v)
            if (v != "") { gsub(/[\[\] ]/, "", v); n=split(v, a, ","); for (i=1;i<=n;i++) if (a[i] != "") print a[i]; inlist=0 } else inlist=1
            next }
        insab && inlist && /^[ \t]*-[ \t]*/ { v=$0; sub(/^[ \t]*-[ \t]*/, "", v); gsub(/["\x27 ]/, "", v); if (v != "") print v; next }
        insab && inlist { inlist=0 }
    ' 2>/dev/null | sort -u
}

# The same set as a JSON object {name: true}
_sablier_names_json() {
    local names; names=$(_sablier_names)
...
_traefik_ensure_plugin() {
    local pname="$1" module="$2" version="$3" line ad cfg
    line=$(_traefik_stack_appdata) || return 0
    ad="${line#*	}"; cfg="$ad/Traefik/traefik.yml"
    [[ -f "$cfg" ]] || return 0
    grep -qF "$module" "$cfg" && return 0
    if grep -qE '^  plugins:' "$cfg"; then
        P="$pname" M="$module" V="$version" awk '{ print } /^  plugins:$/ && !done { printf "    %s:\n      moduleName: \"%s\"\n      version: \"%s\"\n", ENVIRON["P"], ENVIRON["M"], ENVIRON["V"]; done=1 }' "$cfg" > "$cfg.tmp" && mv -f "$cfg.tmp" "$cfg"
    else
...
_sablier_route_files() {
    ...
    [[ -n "$dir" ]] || return 0
    grep -rlE "url: *\"?https?://${name}:[0-9]+" "$dir" --include='*.yml' --include='*.yaml' 2>/dev/null | grep -v -- '-sablier.yml$' | sort
}

# POST /containers/{container}/sablier — Start this container on demand through Sablier (enabled: true) or serve it normally again; writes or removes the Traefik middleware on its route
handle_container_sablier() {
    local name="$1" body="${2:-{\}}"
    if ! _api_check_admin; then _api_error 403 "Admin access required"; return; fi
    docker inspect "$name" >/dev/null 2>&1 || { _api_error 404 "Container not found: $name"; return; }
    printf '%s' "$body" | jq -e 'type == "object"' >/dev/null 2>&1 || { _api_error 400 "Request body must be a JSON object"; return; }
    local enabled session display theme
    enabled=$(printf '%s' "$body" | jq -r 'if .enabled == false then "false" else "true" end')
    session=$(printf '%s' "$body" | jq -r '.session // "30m"'); [[ "$session" =~ ^[0-9]+[smh]$ ]] || session="30m"
    display=$(printf '%s' "$body" | jq -r '.display_name // empty'); [[ -n "$display" ]] || display="$name"
    theme=$(printf '%s' "$body" | jq -r '.theme // "hacker-terminal"'); [[ "$theme" =~ ^[a-z0-9-]+$ ]] || theme="hacker-terminal"
    local dir; dir=$(_find_traefik_routes_dir) || dir=""
    [[ -n "$dir" && -d "$dir" ]] || { _api_error 409 "Traefik is not deployed: on-demand start needs the traefik template and a route for the container"; return; }
    local files; files=$(_sablier_route_files "$name")
    [[ -n "$files" ]] || { _api_error 404 "No Traefik route points at $name yet; deploy it with an HTTPS route first"; return; }
    local mw; mw=$(_sablier_mw_name "$name")
    local stack_dir; stack_dir=$(dirname "$(printf '%s\n' "$files" | head -1)")
    local mw_file="$stack_dir/$mw.yml" sab_restarted=false f
    if [[ "$enabled" == "true" ]]; then
        ...
        # The plugin must be declared in Traefik's static config; older installs get it added
        if ! _traefik_ensure_plugin sablier "github.com/acouvreur/sablier" "v1.7.0-beta.15"; then
            docker restart Traefik >/dev/null 2>&1 && sab_restarted=true
        fi
        printf 'http:\n  middlewares:\n    %s:\n      plugin:\n        sablier:\n          names: %s\n          sablierUrl: http://Sablier:10000\n          sessionDuration: %s\n          dynamic:\n            displayName: %s\n            theme: %s\n            showDetails: true\n            refreshFrequency: 5s\n' \
            "$mw" "$name" "$session" "$display" "$theme" > "$mw_file.tmp" && mv -f "$mw_file.tmp" "$mw_file"
        while IFS= read -r f; do
            [[ -n "$f" ]] || continue
            grep -qF "\"$mw\"" "$f" && continue
            # First router in the file: append the middleware to its list, or create the list after the service line
            if grep -qE '^      middlewares:' "$f"; then
                awk -v mw="$mw" 'done==0 && /^      middlewares:/ { print; print "        - \"" mw "\""; done=1; next } { print }' "$f" > "$f.tmp"
            else
                awk -v mw="$mw" 'done==0 && /^      service:/ { print; print "      middlewares:"; print "        - \"" mw "\""; done=1; next } { print }' "$f" > "$f.tmp"
            fi
            mv -f "$f.tmp" "$f"
        done <<< "$files"
        _audit_log "SABLIER_ENABLED" "$name middleware=$mw session=$session" 2>/dev/null || true
    else
        while IFS= read -r f; do
            [[ -n "$f" ]] || continue
            grep -qF "\"$mw\"" "$f" || continue
            awk -v mw="$mw" '$0 ~ ("^        - \"" mw "\"$") { next } { print }' "$f" > "$f.tmp" && mv -f "$f.tmp" "$f"
        done <<< "$files"
        rm -f "$mw_file" 2>/dev/null
        _audit_log "SABLIER_DISABLED" "$name" 2>/dev/null || true
    fi
    touch "$dir/.reload" 2>/dev/null || true
    _api_success "{\"container\": \"$(_api_json_escape "$name")\", \"enabled\": $enabled, \"middleware\": \"$mw\", \"route_file\": \"$(_api_json_escape "$(printf '%s\n' "$files" | head -1)")\", \"traefik_restarted\": $sab_restarted, \"message\": \"$([[ "$enabled" == "true" ]] && echo "Traefik now starts $name on the first request and Sablier stops it after $session idle" || echo "$name is served normally again")\"}"
}
...
# access log, Discord alerts when a webhook is known, and the Traefik bouncer
# (a LAPI bouncer key, the middleware file, the chain). Runs in the deploy's
# background job, so it may wait for the container.
_crowdsec_post_deploy() {
    local tdir="$1" target_stack="$2" target_dir="$3" deploy_vars="$4" log="$5"
    local i
    for i in $(seq 1 60); do
        [[ "$(docker inspect -f '{{.State.Health.Status}}' CrowdSec 2>/dev/null)" == "healthy" ]] && break
        sleep 2
    done
    docker inspect CrowdSec >/dev/null 2>&1 || { echo "[dcs] CrowdSec container missing, post-deploy skipped" >> "$log"; return 0; }
    docker exec CrowdSec mkdir -p /etc/crowdsec/acquis.d 2>/dev/null || true
    docker cp "$tdir/files/acquis-traefik.yaml" CrowdSec:/etc/crowdsec/acquis.d/traefik.yaml 2>>"$log" && echo "[dcs] CrowdSec reads Traefik's access log" >> "$log"
    # Discord: the deploy's value, else the server's webhook (a secret reference is resolved)
    local webhook domain
    webhook=$(printf '%s\n' "$deploy_vars" | sed -n 's/^DISCORD_WEBHOOK_URL=//p' | head -1)
    ...
    if [[ "$webhook" == https://discord.com/api/webhooks/* || "$webhook" == https://discordapp.com/api/webhooks/* ]]; then
        ...
        docker cp "$tmp" CrowdSec:/etc/crowdsec/notifications/http.yaml 2>>"$log" && docker cp "$tdir/files/profiles.yaml" CrowdSec:/etc/crowdsec/profiles.yaml 2>>"$log" && echo "[dcs] CrowdSec alerts go to Discord" >> "$log"
        rm -f "$tmp"
    fi
    # Traefik bouncer
    local want_bouncer
    want_bouncer=$(printf '%s\n' "$deploy_vars" | sed -n 's/^ENABLE_TRAEFIK_BOUNCER=//p' | head -1); [[ -n "$want_bouncer" ]] || want_bouncer="true"
    if [[ "$want_bouncer" == "true" ]] && grep -q 'container_name: Traefik\|image: traefik' "$target_dir/docker-compose.yml" 2>/dev/null; then
        local dir key lan
        dir=$(_find_traefik_routes_dir) || dir=""
        if [[ -n "$dir" ]]; then
            docker exec CrowdSec cscli bouncers delete dcs-traefik-bouncer >/dev/null 2>&1 || true
            key=$(docker exec CrowdSec cscli bouncers add dcs-traefik-bouncer -o raw 2>>"$log" | tail -1 | tr -d '\r\n ')
            if [[ "$key" =~ ^[A-Za-z0-9+/=_-]{20,}$ ]]; then
                lan=$(grep -m1 '^TRAEFIK_TRUSTED_LAN=' "$target_dir/.env" 2>/dev/null | cut -d= -f2- | tr -d '"' | tr -d "'"); [[ -n "$lan" ]] || lan="192.168.1.0/24"
                mkdir -p "$dir/$target_stack"
                K="$key" L="$lan" awk '{ gsub(/__LAPI_KEY__/, ENVIRON["K"]); gsub(/__TRUSTED_LAN__/, ENVIRON["L"]); print }' "$tdir/files/bouncer-middleware.yml" > "$dir/$target_stack/crowdsec-bouncer.yml"
                ...
                # An install whose traefik.yml predates the plugin list would drop every
                # route in the chain: declare the plugin and restart Traefik once
                ...
                echo "[dcs] could not register the Traefik bouncer (cscli gave no key)" >> "$log"
            fi
        fi
    fi
    docker restart CrowdSec >>"$log" 2>&1 || true
}
...
handle_template_deploy() {
    ...
    if [[ ! -d "$target_dir" || ! -f "$target_dir/docker-compose.yml" ]]; then
        ...
        available_stacks=$(_api_get_stacks)
        [[ -n "$available_stacks" ]] && available_stacks=" Stacks with a compose file: ${available_stacks}"
    ...
    if [[ -d "$tdir/config" ]]; then
        ...
        if [[ -n "$config_target_name" ]]; then
            ...
            # Cloudflare token selects DNS-01, no token the HTTP challenge
            if [[ -f "$config_target/traefik.yml" ]] && grep -q '# dcs-challenge: dns' "$config_target/traefik.yml" 2>/dev/null; then
                local _cf_tok
...
" > "$target_dir/docker-compose.yml" 2>/dev/null || true
    ...
    if [[ "$auto_start" == "true" ]]; then
        ...
        (
            ...
            if [[ "$name" == "crowdsec" && "$_up_ok" == "true" ]]; then
                _crowdsec_post_deploy "$tdir" "$target_stack" "$target_dir" "$vars" "$_activity_log"
            fi
...
handle_template_undeploy() {
    ...
    if [[ "$name" == "crowdsec" ]]; then
        # The bouncer middleware and its place in the chain go with the template
        local _rd; _rd=$(_find_traefik_routes_dir) || _rd=""
        [[ -n "$_rd" ]] && rm -f "$_rd/$target_stack/crowdsec-bouncer.yml" 2>/dev/null && _traefik_chain_set crowdsec-bouncer remove
    fi
...
handle_request() {
    ...
    if [[ "$method" == "GET" ]]; then
        ...
        case "$path" in
            ...
            /routes/certificates)       handle_routes_certificates ;;
    ...
    if [[ "$method" == "POST" ]]; then
        ...
        case "$path" in
            ...
            /containers/*/sablier)
                local container="${path#/containers/}"
                container="${container%/sablier}"
                _api_validate_resource_name "$container" "container" || return
                handle_container_sablier "$container" "$request_body"
                ;;
            /containers/*/env)
                local container="${path#/containers/}"
